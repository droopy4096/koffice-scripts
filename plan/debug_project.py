...
def printState( node, schedule ):
    if node.type() in [ 'Task' ]:
        st = proj.data( node, 'NodeStatus', 'EditRole', schedule )
        print "%-30s %-20s %20s" % ( 
            proj.data( node, 'NodeName', 'DisplayRole', schedule ),
            proj.data( node, 'NodeStatus', 'DisplayRole', schedule ),
            state( int( st ) ) )
...
def printNode( node, props, schedule, types = None ):
    if types is None or node.type() in types:
        for prop in props:
            print "%-25s" % ( proj.data( node, prop[0], prop[1], schedule ) ),
        print

def printGroup( group, props ):
    for prop in props:
        print "%-25s" % ( proj.data( group, prop ) ),
    print
...
def printResource( resource, props ):
    for prop in props:
        print "%-25s" % ( proj.data( resource, prop ) ),
    print
...
def printBusyinfo( res, lst ):
    name = proj.data( res, 'ResourceName' )
    for interval in lst:
